...
```{r eval=FALSE}
####    READ DATA ####
devon_zones = pct::get_pct_zones(region = region_name, geography = "msoa") # get zone data
exeter_zones = devon_zones %>% filter(lad_name == "Exeter") %>% select(geo_code) # filter for exeter

exeter_commute_od = pct::get_pct_lines(region = region_name, geography = "msoa") %>% # get commute od data
...
Data in the PCT for England and Wales is divided into regions, which can be seen here.
