...
montlake_zone_tbl <- dplyr::right_join(all_zones_tbl,
                                        tibble::tibble("TAZ" = unique(c(od_tbl_long$otaz, od_tbl_long$dtaz))),
                                        by = "TAZ") %>%
  dplyr::select(TAZ) %>%
  dplyr::rename(id = TAZ)
...
# buildings = montlake_osm_buildings
# scenario_name = "montlake_test"
...
ab_scenario(
  od = montlake_od_tbl,
  zones = montlake_zone_tbl,
  buildings = sf::st_make_valid(montlake_osm_buildings),
  scenario_name = "montlake_test",
  time_fun = ab_time_normal,
  output = "json_file",
  modes = c("Walk", "Bike", "Drive", "Transit")
